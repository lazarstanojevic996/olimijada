...
@Component({
  selector: 'app-analiza-korisnika',
  templateUrl: './analiza-korisnika.component.html',
  styleUrls: ['./analiza-korisnika.component.css']
})
export class AnalizaKorisnikaComponent implements OnInit 
{
  ...
  ngOnInit() 
  {
    this.pobedeKorisnika = [];
    this.poraziKorisnika = [];
    this.pobedeKorisnikaUlaz = [];
    this.poraziKorisnikaUlaz = [];
  ...
  DajKorisnika() : void
  {
    ...
    let putanja = this.location.path();
    // ---/admin/pregledKorisnika/2/analizaKorisnika
    let id = +putanja.split('/')[3];

    //console.log("ANALIZA: " +  id);
    this.userService.VratiKorisnika(id).subscribe((x) => 
    {
      this.korisnik = Korisnik.FromJsonOne(x);   
  ...
  DajPobedePorazeNeresenoZaKorisnika()
  {
    ...
    (
        ...
        (resp: any) =>
        {
          ...
          //console.log(this.pobedePoraziNereseno.length);
        ...
        (resp: any) =>
        {
          ...
          else
          {
            for(let i=0;i<this.nizDatuma.length;i++)
            {
              ...
              if(this.nizDatuma[i].getFullYear() == this.datumRejting[j].datum_odigravanja.getFullYear() && this.nizDatuma[i].getMonth()==this.datumRejting[j].datum_odigravanja.getMonth() && this.nizDatuma[i].getDate()==this.datumRejting[j].datum_odigravanja.getDate())
              {
